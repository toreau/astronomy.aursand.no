...
### Infrastructure & ops
- **SLSA-5 E2E:** end-to-end verification of the cloud-driven loop — push → CI multi-arch + attestation (SLSA-1) → dispatch → k8s-research digest-bump gate (SLSA-3) → ArgoCD auto-sync → Kyverno `ImageValidatingPolicy` admits the attested image in the kind cluster (SLSA-4). (Changelog-trigger for the E2E run.)
...
- **eop-ut1 unavailable while USNO ser7 serves implausible data** (`Jobs`): added an **IERS Bulletin A fallback** (`finals2000A.all`, fixed-width parser `ParseFinalsLine`; `LoadEopUt1SamplesAsync` falls back when ser7 is unreachable or `|UT1−UTC| > 2 s`). Verified in prod — ser7 returned `8.0 s`, the fallback staged **19 962 samples** (`20260825`, source `finals`, UT1−UTC −0.0798 s) → `eop-ut1` now active (**5/5 datasets**). `ef43fbc`.
